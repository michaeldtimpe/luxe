...
```bash
launchctl print gui/$(id -u)/com.luxe.mlx     | grep -E 'state|program'
launchctl print gui/$(id -u)/com.luxe.litellm | grep -E 'state|program'
```
...
```bash
tail -n 80 ~/Library/Logs/luxe/mlx.err.log
tail -n 80 ~/Library/Logs/luxe/litellm.err.log
```
...
```bash
tail -f ~/Library/Logs/luxe/litellm.err.log
```
...
```bash
launchctl bootout gui/$(id -u)/com.luxe.litellm
claude-local
```
...
```bash
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.luxe.litellm.plist
```
...
| Claude Code shows "API error" | LiteLLM routing bug or qwen format mismatch | Tail `~/Library/Logs/luxe/litellm.err.log`; check `drop_params: true` is still in `litellm_bridge.yml` |
...
```bash
launchctl bootout gui/$(id -u)/com.luxe.mlx
launchctl bootout gui/$(id -u)/com.luxe.litellm
rm ~/Library/LaunchAgents/com.luxe.mlx.plist
rm ~/Library/LaunchAgents/com.luxe.litellm.plist
