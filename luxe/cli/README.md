...
(`cli/heuristic_router.py`) against the prompt. When the scorer is
...
(`cli/import_graph.py`): for each cited file, up to `max_files` total
...
| calc | `create_tool` + tools auto-matched from the saved library (see `cli/tool_library.py`) |
...
**Task-scoped tool cache.** `cli/tasks/cache.py:ToolCache` memoizes
...
**Client-side tool-arg schema.** `cli/agents/base.py:_validate_args`
