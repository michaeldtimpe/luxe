...
def _tasks_list_recent() -> None:
    from cli.tasks import list_all as _list_all
...
def _tasks_resolve(partial: str | None):
    ...
    from cli.tasks import list_all as _list_all
    from cli.tasks.model import resolve_partial as _resolve
    if partial:
        t = _resolve(partial)
        if not t:
...
def _status_renderable(task) -> Group:
    ...
    exact same layout."""
    from cli.repl.status import _fmt_wall
    status = task.status
    live_hint = ""
    if status == "running":
        if task.is_alive():
...
def _tasks_watch(partial: str | None) -> None:
    ...
    from cli.tasks import load as _load_task
...
def _tasks_save(partial: str | None) -> None:
    ...
    from cli.repl.prompt import _ask_styled
    from cli.tasks.report import build_markdown_report
    task = _tasks_resolve(partial)
    if not task:
        return
...
def _tasks_resume(partial: str | None) -> None:
    ...
    from cli.tasks import reset_incomplete_subtasks, spawn_background
    from cli.tasks.model import persist
    task = _tasks_resolve(partial)
    if not task:
        return
    if task.is_alive():
...
def _tasks_abort(partial: str | None) -> None:
    from cli.tasks import abort_task
    task = _tasks_resolve(partial)
    if not task:
        return
...
def _sync_event_printer(event: dict, verbose: bool = False) -> None:
    ...
    from cli.repl.status import _fmt_clock, _fmt_wall
...
def _tasks_run_sync(goal: str, state: "ReplState", cfg: LuxeConfig) -> None:
    ...
    from cli.tasks import Orchestrator
    from cli.repl.review import _plan_and_persist
...
def _tasks_run_background(goal: str, state: "ReplState", cfg: LuxeConfig) -> None:
    ...
    from cli.tasks import spawn_background
    from cli.tasks.model import persist
    from cli.repl.review import _plan_and_persist
