...
│   ✓ grep 0.42s · 1.2KB
│   → read_file path=luxe_cli/prefs.py
...
`luxe_cli/repo_survey.py`) and pick a task wall tuned to its size tier:
...
(`luxe_cli/heuristic_router.py`) against the prompt. When the scorer is
...
(`luxe_cli/import_graph.py`): for each cited file, up to `max_files` total
...
| calc | `create_tool` + tools auto-matched from the saved library (see `luxe_cli/tool_library.py`) |
...
is in `luxe_cli/tools/browser.py:DEFAULT_BROWSER_ALLOWLIST` —
...
`browser` import + tool merge from `luxe_cli/agents/research.py` and
`luxe_cli/agents/lookup.py`. Or simply set `LUXE_BROWSER_ALLOWLIST=""` to
...
**Task-scoped tool cache.** `luxe_cli/tasks/cache.py:ToolCache` memoizes
...
**Client-side tool-arg schema.** `luxe_cli/agents/base.py:_validate_args`
