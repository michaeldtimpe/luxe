...
GIT_BRIEF_HINT = (
    ...
    "or modify anything.\n\n"
    "BUDGET: the whole brief must fit in about 2,000 characters (roughly 40 "
    "short lines). It is HARD-TRUNCATED at that length, so a brief that "
    "spends its budget on the first section loses the last ones. Prefer "
    "covering EVERY section tersely over covering one thoroughly — three to "
    "six short lines each, no sub-bullets, no code blocks.\n\n"
    "Output ONLY the brief as markdown — start directly with the first "
    "`## ` heading, no preamble, no title, no closing summary — using exactly "
    "these sections, each omitted if you have nothing grounded to say:\n"
