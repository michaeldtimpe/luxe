...
LOG_DIR="$HOME/Library/Logs/luxebox"
BIN_DIR="$HOME/.local/bin"

PLIST_SRC="$REPO_ROOT/daily_driver/launchd/com.luxebox.ollama.plist"
PLIST_DST="$LAUNCH_AGENTS/com.luxebox.ollama.plist"
...
install() {
  ...
  if launchctl list | grep -q "com.luxebox.ollama"; then
    say "reloading existing launchd job"
    launchctl bootout "gui/$UID/com.luxebox.ollama" 2>/dev/null || true
  fi
  launchctl bootstrap "gui/$UID" "$PLIST_DST"
  launchctl enable "gui/$UID/com.luxebox.ollama" || true
