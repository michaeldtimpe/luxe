...
LOGS_DIR="$HOME/Library/Logs/luxebox"
...
mkdir -p "$AGENTS_DIR"
for label in com.luxebox.mlx com.luxebox.litellm; do
...
cat > "$settings_file" <<'JSON'
{
  ...
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "sk-local-luxebox",
...
Next steps:
  ...
  1. Verify the agents are running:
       launchctl print gui/$(id -u)/com.luxebox.mlx | grep state
       launchctl print gui/$(id -u)/com.luxebox.litellm | grep state
  ...
  3. Tail logs while they come up:
       tail -f ~/Library/Logs/luxebox/{mlx,litellm}.err.log
