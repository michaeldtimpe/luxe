"""Tests for the WS2 acted-but-wrong sizing analysis (scripts/_archive/analyze_acted_but_wrong.py).
...
from scripts._archive.analyze_acted_but_wrong import (
    classify_failure,
    norm_eq,
    param_subtype,
    parse_call,
...
_REP = (Path(__file__).resolve().parents[2]
...
def test_substitute_gt_values_overwrites_and_adds():
    ...
    from scripts._archive.verify_wrong_binding_attribution import substitute_gt_values
...
def test_summary_smoke_emits_manifest(tmp_path):
    from scripts._archive.analyze_acted_but_wrong import _run_summary
