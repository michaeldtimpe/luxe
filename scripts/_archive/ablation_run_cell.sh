...
# happens via scripts/_archive/ablation_aggregate.py at the end):
...
#   bash scripts/_archive/ablation_run_cell.sh <cell> [options]
#
...
if [[ $RUN_SWEBENCH -eq 1 ]]; then
  ...
  if [[ $RUN_HARNESS -eq 1 && -f "${SWE_OUT}/predictions.json" ]]; then
    ...
    set +e
    python scripts/_archive/ablation_harness.py \
