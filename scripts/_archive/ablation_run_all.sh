...
#   bash scripts/_archive/ablation_run_all.sh [options-passed-through-to-each-cell]
#
# All options are passed through to ablation_run_cell.sh, so:
#   bash scripts/_archive/ablation_run_all.sh --bfcl-limit 100 --swebench-smoke 25 \
#       --harness-workers 4
...
for cell in "${CELLS[@]}"; do
  ...
  echo "===================================================="
  bash scripts/_archive/ablation_run_cell.sh "$cell" "${PASS_THROUGH[@]}"
  rc=$?
  if [[ $rc -ne 0 ]]; then
...
echo ""
echo "    next: python scripts/_archive/ablation_aggregate.py --root acceptance/agentic_ablation/"
